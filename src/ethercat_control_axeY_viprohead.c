...
// gcc -o ethercat_control_axeY_viprohead ../src/ethercat_control_axeY_viprohead.c -lrt -pthread -L/usr/local/etherlab/lib -lethercat
// --- Pour lancer le code: ---
// sudo LD_LIBRARY_PATH=/usr/local/etherlab/lib ./ethercat_control_axeY_viprohead
...
void cyclic_task()
{
    ...
    if (off_axisY_statusword != 0) {
        ...
        if (axis_status & SW_TARGET_REACHED) {
            if (motion_started && !motion_complete) {
                motion_complete = true;
                printf("\n CIBLE ATTEINTE! Position: %.2f mm\n", y_position_mm);
            }
        }
    }
...
{
    const double seg_n = DIST_AVANCE_VIDE1;
    const double seg_m = DIST_EXTRU_LENTE;
    const double seg_p = DIST_EXTRU_RAPIDE;
    printf("\n[SUPERVISEUR] Démarré\n");
    printf("  Segments: %.1fmm vide | %.1fmm lent | %.1fmm rapide | %.1fmm vide\n",
           seg_n, seg_m, seg_p, DIST_AVANCE_VIDE2);

    while (!stop && !motion_complete) {
        double y = y_position_mm;
        double extr_steps = 0.0;

        if (y < seg_n) {
            extr_steps = 0.0;
        } else if (y < (seg_n + seg_m)) {
            extr_steps = EXTRU_LENTE;
        } else if (y < (seg_n + seg_m + seg_p)) {
            extr_steps = EXTRU_RAPIDE;
        } else {
            extr_steps = 0.0;
        }

        user_velocity_steps_per_s = extr_steps;

        static int display_counter = 0;
        if (display_counter++ % 10 == 0) { // Affichage toutes les secondes
            printf("[SUP] Y=%.2fmm | Extru=%.0f steps/s\n", 
                   y, user_velocity_steps_per_s);
        }
        static double last_extrusion = -1.0;

        if (fabs(extr_steps - last_extrusion) > 0.1) {   // changement détecté
            struct timespec ts;
            ...
            if (log_file) {
                fprintf(log_file, "%f,%.3f,%.1f\n", 
                        t, y, extr_steps);
                fflush(log_file);
            }

            printf("[LOG] Changement extrusion -> %.0f steps/s à Y=%.2fmm\n",
                extr_steps, y);

            last_extrusion = extr_steps;
        }
...
int main(void)
{
    ...
    log_file = fopen("extrusion_events.csv", "w");
    if (!log_file) {
        perror("fopen log");
        return 1;
    }
    fprintf(log_file, "timestamp_s,y_position_mm,extrusion_steps_s\n");
    fflush(log_file);
    ...
    // Après activation et échanges PDO initiaux, on envoie la séquence CiA-402 via PDO
    if (send_controlword_sequence_via_pdo() < 0) {
        fprintf(stderr, "Failed to send controlword sequence via PDO\n");
    }

    // Compute of the total distance
    double total_mm = DIST_AVANCE_VIDE1 + DIST_EXTRU_LENTE + 
                      DIST_EXTRU_RAPIDE + DIST_AVANCE_VIDE2;

    printf("\n=== Trajectory parameters ===\n");
    printf("Total distance: %.1f mm\n", total_mm);
    printf("Axis Y velocity: %.1f mm/s\n", AXEY_VEL_MM_S);
    printf("Estimated trajectory time: %.1f secondes\n", total_mm / AXEY_VEL_MM_S);
...
// Stopping proprely
    ...
    // Quelques cycles pour envoyer vitesse = 0
    for (int i = 0; i < 100; i++) {
        cyclic_task();
        usleep(1000);
    }
