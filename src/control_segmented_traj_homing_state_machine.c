...
static int read_sdo(uint16_t slave, uint16_t index, uint8_t sub, void *target, size_t size) {
    uint32_t abort;
    size_t result_size;
    int ret = ecrt_master_sdo_upload(master, slave, index, sub, (uint8_t *)target, size, &result_size, &abort);
    if (ret < 0) {
...
void cyclic_task() {
    ...
    // --- Mode TRAJECTORY ---
    if (cyclic_mode == CYCLE_SEGMENTED_TRAJECTORY && motion_started && !motion_complete) {
        static int etape = 0;
    ...
    if (cyclic_mode == CYCLE_SHUTDOWN) {
        ...
        if (off_extruder_control_byte != 0) {
            EC_WRITE_U8(domain1_pd + off_extruder_control_byte, 0);
        }
        if (off_extruder_velocity != 0) {
            EC_WRITE_U16(domain1_pd + off_extruder_velocity, 0);
        }
        if (off_axisY_controlword != 0) {
            EC_WRITE_U16(domain1_pd + off_axisY_controlword, 0x0006); // Shutdown
        }
        stop = 1;
    }

    // --- LOG DE LA POSITION Y ---
    if (log_position_file && cyclic_mode != CYCLE_ERROR) {
        struct timespec ts;
        clock_gettime(CLOCK_MONOTONIC, &ts);
        double t = ts.tv_sec + ts.tv_nsec * 1e-9;
...
void *supervisor_thread(void *arg) {
    ...
    while (!stop && !motion_complete && !system_error) {
        usleep(100000);
    }
...
int main(void) {
    ...
    printf("Offsets PDO:Y_pos=%u Y_status=%u Y_cw=%u Y_target=%u Ext_ctl=%u Ext_vel=%u\n",
           off_axisY_pos_actual, off_axisY_statusword, off_axisY_controlword, off_axisY_target,
           off_extruder_control_byte, off_extruder_velocity);
    ...
    while (!stop) {
        ...
        if (cyclic_mode == CYCLE_SEGMENTED_TRAJECTORY && !motion_started) {
            printf("[INFO] Départ trajectoire principale\n");
            axisY_target_pos_um = MM_TO_UM(DIST_AVANCE_VIDE1 + DIST_EXTRU_LENTE + DIST_EXTRU_RAPIDE + DIST_AVANCE_VIDE2);
            motion_started = true;
            motion_complete = false;
        ...
        if (motion_complete || system_error) break;
    }
