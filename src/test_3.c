...
#define EXTRU_LENTE 186.5 // extr. velocity slow
#define EXTRU_RAPIDE 266.43 // extr. velocity fast
...
void cyclic_task()
{
    ...
    if (cyclic_mode == CYCLE_MOVE_TO_ZERO) {
        if (off_axisY_target != 0 && off_axisY_controlword != 0) {
            EC_WRITE_S32(domain1_pd + off_axisY_target, 0);
            EC_WRITE_U16(domain1_pd + off_axisY_controlword, 0x009B); // Start + new setpoint
        }

        // Check if zero reached
        if ((axis_status & SW_TARGET_REACHED) || y <= 0.1) {
            printf("[INFO] Retour à zéro terminé: %.2f mm\n", y);
            cyclic_mode = CYCLE_IDLE; // on passe à idle ou on peut lancer la trajectoire
        }
    }

    // --- Mode TRAJECTORY ---
    if (cyclic_mode == CYCLE_TRAJECTORY && motion_started && !motion_complete) {
...
/* -------------------------------------------------------------------------- */
void move_to_zero() {
    if (!domain1_pd || off_axisY_target == 0 || off_axisY_controlword == 0) {
        fprintf(stderr, "Offsets PDO invalides pour move_to_zero()\n");
        return;
    }
...
int main(void)
{
    ...
    while (!stop) {
        ...
        // --- Si zéro atteint et trajectoire non démarrée, lancer trajectoire ---
        if (cyclic_mode == CYCLE_IDLE && !motion_started) {
