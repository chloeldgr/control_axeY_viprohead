...
# EtherCAT Z-Axis Control (Bus Position 0)
# Wrapper script that calls ethercat_control_single_axis.sh with bus_position=0
...
usage() {
    ...
    echo ""
    echo "This script controls the Z-axis slave at bus position 0"
    echo ""
    echo "Examples:"
...
echo "Z-Axis Control: Calling ethercat_control_single_axis.sh with bus_position=0"
echo ""

# Call the main script with bus_position=0 and all provided arguments
exec "$SCRIPT_DIR/ethercat_control_single_axis.sh" --bus_position=0 --shutdown_at_the_end "$@"